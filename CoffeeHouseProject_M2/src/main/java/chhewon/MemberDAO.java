...
public class MemberDAO {
	...
	public ArrayList<ZipcodeDTO> zipcodeRead(String area3) {
		...
		try {
			...
			//검색된 레코드의 값을 필드별로 담는 소스코드->찾은 레코드가 1개->if(rs.next())
			//												1개 이상->while(rs.next())
			while (rs.next()) {
			...
			System.out.println("zipcodeRead() 에러발생=>"+e);
		}finally {
			pool.freeConnection(con, pstmt, rs);
		}
		return zipList;
	...
	public boolean memberInsert(MemberDTO mem) {
		...
		try {
			...
			pstmt.setString(8, mem.getMem_prefer());
	...
	public boolean memberUpdate(MemberDTO mem) {
		...
		try {
			...
			//-----------------------
			sql="update chmember set passwd=?, name=?, email=?, phone=?, zipcode=?, address=?, prefer=? where id=?";
			pstmt=con.prepareStatement(sql);
