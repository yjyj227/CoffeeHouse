...
public class DBConnectionMgr {
    ...
    //(2) dboracle.properties파일을 읽어들여서 키->값을 불러오기
    public DBConnectionMgr() throws Exception {
    	Properties props=new Properties();
    	FileInputStream in=
    			new FileInputStream("C:\\webtest\\4.jsp\\2.back-end\\sou2\\CoffeeHouseProject_M2\\src\\main\\webapp\\dbtest\\dboracle.properties");
    	props.load(in); //파일의 내용을 메모리에 불러오기
    	in.close();
    	_driver=props.getProperty("jdbc.drivers");
    	if (_driver!=null) System.setProperty("jdbc.drivers", _driver); //등록(클래스 이름)
    	//-------------------------------------------------------------------------------
