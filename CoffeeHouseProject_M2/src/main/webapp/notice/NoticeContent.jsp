...
.comtable{
	border-top:1.5px solid rgb(206, 151, 139);
	border-bottom:1px solid rgb(206, 151, 139);
}
.contentarea {
	line-height:3.5rem;
}
  ...
  <body>
	...
	<a class="topbutton" href="#"><img src="img/topbutton.png"></a>
		...
		<div class="row">
			...
			<table class="table">
					...
					<tr>
						<td>
							...
							<div class="col-md-2"><fmt:formatDate value="${article.reg_date}" timeStyle="medium" pattern="yy.MM.dd  hh:mm" /></div>
							<div class="col-md-1">${article.readcount}</div>
						</td>
					</tr>
					...
					<tr>
						...
						<%} %> --%>
						<%-- <c:choose>
							<c:when test="${article.filename!=null || article.filename!=('')}">
								&nbsp;&nbsp;&nbsp;&nbsp;첨부파일&nbsp;&nbsp;&nbsp;<a href="javascript:down('${article.filename}')">${article.filename}</a>
								&nbsp;&nbsp;<font-color="gray">(${article.filesize}KBytes)</font-color>
							</c:when>
							<c:otherwise>
								&nbsp;&nbsp;&nbsp;&nbsp;첨부된 파일이 없습니다.
							</c:otherwise>
						...
						<c:choose>
						<c:when test="${(!empty article.filename) || article.filename!=('')}">
							&nbsp;&nbsp;&nbsp;&nbsp;첨부파일&nbsp;&nbsp;&nbsp;<a href="javascript:down('${article.filename}')">${article.filename}</a>
							&nbsp;&nbsp;
						</c:when>
						<c:when test="${(empty article.filename) || article.filename==('')}">
					...
					<tr>
						<td>
						<div class="col-md-12 contentarea" align="left">${fn:replace(article.content, replaceChar, "<br/>")}</div>
						</td>
					</tr>
					<tr>
						<td>
						...
						<button type="button" class="btn btn-default" onclick="document.location='NoticeList.do?pageNum=${pageNum}'">목록으로</button>
						</div>
						<div class="col-md-2"></div>
						<div class="col-md-8" align="right">
						<%-- <%
						if (mem_id!=null) {
							if (article.getId()==mem_id || mem_id.equals("admin")) {%> --%>
					<%-- <c:if test="${mem_id!=null}">
						<c:if test="${article.id==mem_id || mem_id==('admin')}"> --%>
					<c:if test="${!empty mem_id}">
						
						<c:if test="${article.id==mem_id || mem_id.equals('admin')}">
						<button type="button" class="btn btn-warning" onclick="document.location='NoticeUpdateForm.do?num=${article.num}&pageNum=${pageNum}'">수정</button>
						<button type="button" class="btn btn-danger" onclick="document.location='NoticeDeleteForm.do?num=${article.num}&pageNum=${pageNum}'">삭제</button>
						</c:if>
						
						<button type="button" class="btn btn-default" onclick="document.location.href='NoticeWriteForm.do?num=${article.num}&ref=${article.ref}&re_step=${article.re_step}&re_level=${article.re_level}'">답글쓰기</button>
					</c:if>
			...
			<div class="col-md-12" align="left">댓글 (2)</div>
			<table class="table comtable">
				<tr>
					<td>
						<div class="col-md-2" align="left">admin</div>
						<div class="col-md-2" align="left"><font size="1.5rem">22.02.26 06:38</font></div>
						<div class="col-md-8"></div>
						<div class="col-md-12" align="left">댓글내용 댓글내용 댓글내용 댓글내용 댓글내용 댓글내용 댓글내용 댓글내용 댓글내용 댓글내용</div>
						<div class="col-md-12" align="left">
							<button type="button" class="btn btn-default btn-xs">대댓글</button>
				...
				<tr>
					<td>
						<div class="col-md-2" align="left"><img src="img/level.gif" width="10" height="1">┗ admin</div>
						<div class="col-md-2" align="left"><img src="img/level.gif" width="10" height="1"><font size="1.5rem">22.02.26 06:56</font></div>
						<div class="col-md-8"></div>
						<div class="col-md-12" align="left"><img src="img/level.gif" width="10" height="1">댓글내용2 댓글내용2 댓글내용2 댓글내용2 댓글내용2 댓글내용2 댓글내용2 댓글내용2 댓글내용2 댓글내용2</div>
						<div class="col-md-12" align="left">
							<img src="img/level.gif" width="10" height="1">
							<button type="button" class="btn btn-default btn-xs">대댓글</button>
							<button type="button" class="btn btn-default btn-xs">수정</button>
							<button type="button" class="btn btn-default btn-xs">삭제</button>
						</div>
